...
/// Internal class used in both `Multiplexer` and `MultiplexerMap`
@MuxActor
public class _MuxFetcher<T: Codable & Sendable> {
    ...
    private var isExpired: Bool { Date().timeIntervalSinceReferenceDate > completionTime + defaultTTL }


    internal func request(domain: String?, key: LosslessStringConvertible?, onFetch: @escaping OnFetch) async throws -> T {
...
///
@MuxActor
public final class Multiplexer<T: Codable & Sendable>: _MuxFetcher<T>, MuxRepositoryProtocol {
    ...
    public func request() async throws -> T {
        return try await request(domain: muxRootDomain, key: cacheKey, onFetch: onFetch)
    }
